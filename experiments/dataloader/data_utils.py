...
class LabelNoise(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.p[idx]:
            label = int(label - label % self.k + self.noise[idx])
...
class Subset(data.Dataset):
    def __init__(self, dataset, indices=None):
        ...
        if self.indices is None:
            ...
            assert 0 <= self.n_samples <= len(self.dataset), \
                "length of {} incompatible with dataset of size {}".format(self.n_samples, len(self.dataset))
...
def split_dataset(dataset_train, dataset_val, train_size, val_size):
    if isinstance(dataset_train, datasets.ImageFolder):
        ...
        if train_size < len(dataset_train):
            ...
            print('train_size_per_class : ', train_size_per_class)
        else:
        ...
        dataset_train.targets = [s[1] for s in train_imgs]
